...
import { getAuthErrorMessage, getCurrentUser, loginWithGoogle, registerWithEmail } from "@/services/auth-service";

const registerSchema = z.object({
  name: z.string().min(2, "Magacu ugu yaraan waa 2 xaraf."),
...
export default function RegisterPage() {
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    startTransition(() => {
      void registerWithEmail(result.data)
        .then(() => {
          router.replace("/chat");
          router.refresh();
        })
  ...
  return (
    <AuthShell
      title="Is diiwaangeli"
      description="Samee akoon GARAS Chat si aad u kaydsato wada sheekaysiyada Somali AI Assistant-ka."
      alternateLabel="Akoon hore ma leedahay?"
      alternateHref="/login"
      alternateText="Soo gal"
    >
