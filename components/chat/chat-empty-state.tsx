...
export function ChatEmptyState({ quickPrompts, onSelectPrompt, composer }: ChatEmptyStateProps) {
  return (
    <section className="mx-auto flex min-h-[60vh] w-full max-w-4xl flex-col items-center justify-center px-3 pb-4 text-center sm:px-4">
      ...
      <h1 className="mt-6 max-w-3xl text-balance text-4xl font-semibold tracking-tight text-white sm:text-5xl">
      ...
      <div className="mt-6 w-full max-w-[680px]">{composer}</div>

      <div className="mt-5 w-full max-w-[920px] px-1">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {quickPrompts.map((prompt, index) => (
            <button
              ...
              className="group inline-flex w-full items-center justify-start gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2.5 text-left transition hover:-translate-y-0.5 hover:border-sky-300/30 hover:bg-sky-300/10"
            >
              {index % 2 === 0 ? (
