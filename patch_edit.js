...
if (code.includes(oldUserBlock)) {
    code = code.replace(oldUserBlock, newUserBlock);
    fs.writeFileSync('components/chat/message-bubble.tsx', code);
    console.log('SUCCESS');
} else {
    // If exact spaces or tabs don't match, try regex or simpler replace
    console.log('Failed to match exact codeblock, wait..');
    
    // Fallback: splitting by "isUser ? (" and ") : ("
    const startIdx = code.indexOf('{isUser ? (');
    const roleFallback = code.indexOf('          <div className="min-w-0 max-w-full text-sm leading-7 text-slate-200 wrap-anywhere">');
    if (startIdx !== -1 && roleFallback !== -1) {
       const finalStr = code.substring(0, startIdx) + newUserBlock + '\n' + code.substring(roleFallback);
       fs.writeFileSync('components/chat/message-bubble.tsx', finalStr);
       console.log("SUCCESS FALLBACK");
    } else {
       console.log("FAIL COMPELETELY");
    }
}
